...
from xgoogle.search import BeautifulSoup
import nltk

from xgoogle.search import Browser, BrowserError
...
class GoogleSearch(object):
    ...
    def __init__(self, query, random_agent=False, debug=False, lang="en", tld="com", re_search_strings=None):
        ...
        if re_search_strings:
            ...
            self._re_search_strings = ("Omtrent", "resultater")
        # add more localised versions here
        else:
            self._re_search_strings = ("About", "results")
    ...
    def _extract_info(self, soup):
        ...
        txt = txt.replace(',', '')
        matches = re.search(r'%s ([\d\W]*) %s' % self._re_search_strings, txt, re.U)

        if not matches:
            return empty_info
        return {'from': 0, 'to': 0, 'total': int(re.sub("&#\d+;", "", matches.group(1)))}
